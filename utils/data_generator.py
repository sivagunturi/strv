...
class DataGenerator:
    ...
    def generate_data(self, number_of_images, image_size=None, save_to=None):
        ...
        print("get files face", self.get_files_faces())
        print("no of images", number_of_images)
        
        image_files = self.get_files_faces()
        print("no of files => ",len(image_files))
        if(len(image_files) < number_of_images):
            number_of_images = len(image_files)

        for i, file in tqdm(enumerate(random.sample(image_files, number_of_images)), total=number_of_images):
            # Load images
            if(exists(file)):               
